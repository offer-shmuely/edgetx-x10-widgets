...
local function onTelemetryResetEvent(wgt)
  wgt.resetCount = wgt.resetCount + 1
...
local function detectResetEvent(wgt)
  ...
  if (currMinRSSI == wgt.lowestMinRSSI) then return end

  if (currMinRSSI < wgt.lowestMinRSSI) then
    -- rssi just got lower, record it
    wgt.lowestMinRSSI = currMinRSSI
    return
  end


  -- reset telemetry detected
  wgt.lowestMinRSSI = 101
...
function refresh(wgt)
  ...
  lcd.drawText(wgt.zone.x, wgt.zone.y + 15, "lowestMinRSSI: ".. wgt.lowestMinRSSI, SMLSIZE + CUSTOM_COLOR)
  lcd.drawText(wgt.zone.x, wgt.zone.y + 30, string.format("resetCount: %d", wgt.resetCount), SMLSIZE + CUSTOM_COLOR)
...
return { name = "t_fieldInfo2", options = options, create = create, update = update, background = background, refresh = refresh }
