...
local function getV(wgt, id)
    ...
    if cid == nil then
        ...
        vcache[id] = cid
        log("getV(%s) cache updated- cid: %d", id, cid)
    end

    local v = cid ~= 0 and getValue(cid) or nil
    log("getV(%s) return - val: %d", id, v)
    return v
end
...
local function drawLq(wgt, Y)
    --lcd.drawText(5, Y, string.format("LQ: %d %%", wgt.tlm.rqly), FS.FONT_8 + TXT_COL)
    lcd.drawText(5, Y, "Link Qly: ", FS.FONT_8 + TXT_COL)
...
local function drawDbgText(msg)
    lcd.drawText(dbgX, dbgY, msg, FS.FONT_6 + LIGHTGREY + RIGHT)
    dbgY = dbgY +dbgDx
end
...
local function refresh(wgt, event, touchState)
    ...
    --    if wgt.zh > 50 then
    --        lcd.drawText(wgt.zw / 2, 10, "-- ELRS-RF --", FS.FONT_8 + WHITE + CENTER)
    --    end
    --    lcd.drawText(wgt.zw / 2, wgt.zh / 2, "No RX connected...", FS.FONT_12 + WHITE + CENTER+ VCENTER)
    --    return
    --end
    ...
    if wgt.zh > 200 then
        local y = 180
        local dy = 20
        lcd.drawText(10, y, string.format("RX link quality min: %s%%",wgt.tlm.rqly_min), FS.FONT_8 + TXT_COL)
        y = y + dy
        lcd.drawText(10, y, string.format("TX power         max: %smW",wgt.tlm.tpwr_max), FS.FONT_8 + TXT_COL)
